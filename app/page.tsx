...
import ParallaxMeter from "@/components/ParallaxMeter";
...
const FEATURES = [
  {
    number: "N\u00B001",
    title: "Curated spots",
    body: "From Ha'penny Bridge to Howth Cliffs \u2014 genre, light and access info for every location.",
    delay: "0.05s",
  },
  {
    number: "N\u00B002",
    title: "Data-driven",
    body: "220 fictional session records show how time of day, weather and crowds affect results.",
    delay: "0.12s",
  },
  {
    number: "N\u00B003",
    title: "Community input",
    body: "Know a hidden gem? Suggest a new spot \u2014 no account or personal data required.",
    delay: "0.19s",
  },
];
    ...
    <div>
      ...
      <section className="grid gap-6 border-t border-[var(--frame)] pt-14 sm:grid-cols-3">
        {FEATURES.map((feature) => (
          ...
          >
            <p className="font-mono-data text-xs text-[var(--safelight)]">
              {feature.number}
            </p>
            <h2 className="mt-2 font-display text-lg text-[var(--paper)]">
              {feature.title}
            </h2>
            <p className="mt-2 text-sm text-[var(--ash)]">{feature.body}</p>
          </div>
        ))}
      </section>
    </div>
